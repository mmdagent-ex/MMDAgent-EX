...
PaError Pa_OpenSLES_Initialize()
{
   ...
   __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_initialize");
...
PaError Pa_OpenSLES_Terminate()
{
   __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_Terminate");

   Pa_OpenSLES_StopStream((PaStream *) '\x01');
   Pa_OpenSLES_StopStream((PaStream *) '\x02');
   SLresult result1 = Pa_OpenSLES_CloseStream((PaStream *) '\x01');
   SLresult result2 = Pa_OpenSLES_CloseStream((PaStream *) '\x02');
...
PaError Pa_OpenSLES_OpenStream(PaStream** stream, const PaStreamParameters *inputParameters, const PaStreamParameters *outputParameters, double sampleRate, unsigned long framesPerBuffer, PaStreamFlags streamFlags, PaStreamCallback *streamCallback, void *userData)
{
   ...
   if(outputParameters != NULL) {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_OpenStream 01");
      /* reset */
      Pa_OpenSLES_CloseStream((PaStream *) '\x01');
   ...
   if(inputParameters != NULL) {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_OpenStream 02");
      /* reset */
      Pa_OpenSLES_CloseStream((PaStream *) '\x02');
...
FlushPlayerQueue()
{
   ...
   if (currentPlayerBufferSize != 0) {
      ...
      currentPlayerBufferSize = 0;
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_WriteStream callback flush %u", (unsigned int)length);
      numQueuedPlayerBuffer++;
...
PaError Pa_OpenSLES_CloseStream(PaStream *stream)
{
   if(stream == (PaStream *) '\x01') {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_CloseStream 01");
      FlushPlayerQueue();
      if (player != NULL)
   ...
   if(stream == (PaStream *) '\x02') {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_CloseStream 02");
      if (recorder != NULL)
...
PaError Pa_OpenSLES_StartStream(PaStream *stream)
{
   ...
   if(stream == (PaStream *) '\x01' && player != NULL) {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_StartStream 01");
   ...
   if(stream == (PaStream *) '\x02' && recorder != NULL) {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_StartStream 02");
...
PaError Pa_OpenSLES_StopStream(PaStream *stream)
{
   ...
   if(stream == (PaStream *) '\x01' && player != NULL) {
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_StopStream 01");
      SLPlayItf playerInterface;
      result = (*player)->GetInterface(player, SL_IID_PLAY, &playerInterface);
      if (result != SL_RESULT_SUCCESS) {
         Pa_OpenSLES_Terminate();
   ...
   if(stream == (PaStream *) '\x02' && recorder != NULL) {
      SLRecordItf recorderInterface;
      __android_log_print(ANDROID_LOG_VERBOSE, "MMDAgent", "Pa_OpenSLES_StopStream 02");
      result = (*recorder)->GetInterface(recorder, SL_IID_RECORD, &recorderInterface);
      if (result != SL_RESULT_SUCCESS) {
         Pa_OpenSLES_Terminate();
         return paInternalError;
